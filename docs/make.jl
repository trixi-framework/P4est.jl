...
open(joinpath(@__DIR__, "src", "license.md"), "w") do io
    ...
    for line in eachline(joinpath(dirname(@__DIR__), "LICENSE.md"))
        line = replace(line,
                       "[AUTHORS.md](AUTHORS.md)" => "[Authors](@ref authors_separate_page)")
        println(io, "> ", line)
    end
end
...
if (get(ENV, "CI", nothing) != "true") &&
   (get(ENV, "JULIA_P4EST_DOC_DEFAULT_ENVIRONMENT", nothing) != "true")
...
deploydocs(repo = "github.com/trixi-framework/P4est.jl",
           devbranch = "main",
