...
set up correctly by the following extended smoke test.
...
julia> MPI.Barrier(MPI.COMM_WORLD)

julia> rank = MPI.Comm_rank(MPI.COMM_WORLD)

julia> @info "Setup" rank p4est_obj.local_num_quadrants p4est_obj.global_num_quadrants
┌ Info: Setup
│   rank = 0
